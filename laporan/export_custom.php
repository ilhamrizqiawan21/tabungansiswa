...
// Set judul laporan
$sheet->setCellValue('A1', 'LAPORAN TRANSAKSI TABUNGAN SISWA');
...
if (empty($transaksi)) {
    ...
    $rekapitulasi = [
        ['Total Setoran:', $total_masuk],
        ['Total Penarikan:', $total_keluar],
        ['Saldo Bersih:', $total_masuk - $total_keluar]
    ];
    
    foreach ($rekapitulasi as $rekap) {
        $sheet->setCellValue('A' . $row, $rekap[0]);
        $sheet->setCellValue('B' . $row, $rekap[1]);
...
$filename = 'Laporan_Tabungan_' . date('d_m_Y', strtotime($start_date)) . '_sd_' . date('d_m_Y', strtotime($end_date)) . '.xlsx';
